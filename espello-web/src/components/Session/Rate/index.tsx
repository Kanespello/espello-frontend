    ...
    const submitFeedback = async () => {
        ...
        try {
            ...
            if (result.status === 'success') {
                ...
                setMessage({ text: 'An unknown error occurred, please retry.', type: 'error' });
            }
        }
        catch (error: any) {
            console.error('Error:', error);
            setMessage({ text: 'An error occurred while submitting feedback. Please try again later.', type: 'error' });
        }
    }
            ...
            <div className='rate-container'>
                ...
                <div className='rate-container-feedback'>
                    <div className='rate-container-feedback-rate'>
                        {
                            Array.from({ length: 5 }, (_, index) => (
                                rating >= (index + 1) ?
                                    <svg key={index} xmlns="http://www.w3.org/2000/svg" width="26" height="25" viewBox="0 0 26 25" fill="none" onClick={() => updateRating(index)}>
                    ...
                    <div className='rate-container-feedback-content'>
                        <textarea className='rate-container-feedback-content-input' placeholder='' rows={5} value={comment} onChange={(event: React.ChangeEvent<HTMLTextAreaElement>) => {
                            setComment(event.target.value);
                        }} 
                        maxLength={250}/>
                    </div>
                    {message && (
                        <div className={`rate-container-feedback-${message.type}`}>
                            {message.text}
                        </div>
                    )}
                </div>
